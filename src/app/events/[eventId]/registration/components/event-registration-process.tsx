...
export const EventRegistrationProcess = ({ event, competitions, attendee }: IEventRegistrationProcess) => {
  ...
  const nextButtonDisabled = (): boolean => {
    if (!page) {
      // participant deadline exceeded
      if (moment(event?.registrationDeadline).unix() < moment().unix()) {
  ...
  return (
    <div className="min-h-0 flex-1 flex flex-col">
      ...
      >
        <TextareaAutosize readOnly className="h-[90vw] w-[90vw] resize-none overflow-hidden bg-transparent outline-hidden" value={event.waiver} />
      </Dialog>
