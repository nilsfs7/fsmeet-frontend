...
export const CompetitionList = ({
  ...
  const getSelectionHint = (comp: Competition, index: number): string | undefined => {
    if (!selectable || !disabled[index]) return undefined;

    if (comp.isFollowUpCompetition) {
      return t('hintFollowUpCompetition');
    }

    // Eligibility first — more useful than “host disabled registration”.
    if (comp.gender !== CompetitionGender.MIXED && comp.gender !== userGender) {
      return t('hintGenderMismatch', { gender: comp.gender });
    }

    if (comp.maxAge > 0 && (!userAge || comp.maxAge < userAge)) {
      return t('hintMaxAgeMismatch', { maxAge: comp.maxAge });
    }

    if (comp.registrationEnabled === false) {
      return t('hintRegistrationDisabledByHost');
    }

    if (comp.id) {
      const amountRegs = amountRegistrations[index];
      if (comp.maxAmountParticipants > 0 && amountRegs != null && comp.maxAmountParticipants <= amountRegs) {
        return t('hintCompetitionFull');
      }
    }
          ...
          <table className="w-full min-w-0 text-sm text-foreground">
            <thead className="bg-muted/80 text-foreground/90">
              <tr className="text-left text-xs font-medium uppercase leading-normal tracking-wide">
                ...
                <th className="min-w-0 max-w-48 py-2.5 px-1 sm:max-w-none sm:px-2 sm:py-3">{t('columnTitleName')}</th>
            ...
            <tbody>
              {competitions.map((comp, i) => {
                const selectionHint = getSelectionHint(comp, i);
                const checkbox = (
                  <Checkbox
                ...
                return (
                  <tr key={i} className="border-b border-border/50 last:border-b-0 hover:bg-muted/25 dark:hover:bg-muted/20">
                    ...
                    <td className="min-w-0 max-w-48 wrap-break-word py-2.5 px-1 sm:max-w-none sm:px-2 sm:py-3">
