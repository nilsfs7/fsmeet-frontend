...
export async function getUser(username: string, session?: Session | null): Promise<User> {
  ...
  if (response.ok) {
    ...
    const user: User = {
      ...
      jobProfileState: data.job?.profileState,
      jobOfferShows: data.job?.offerShows,
      jobOfferWalkActs: data.job?.offerWalkActs,
      jobOfferWorkshops: data.job?.offerWorkshops,
