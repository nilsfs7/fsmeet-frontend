...
export async function createSponsor(eventId: string, name: string, session: Session | null): Promise<void> {
  const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/sponsors`;

  const body = new CreateSponsorBodyDto(eventId, name);
...
export async function updateSponsor(id: string, name: string, session: Session | null): Promise<void> {
  const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/sponsors`;

  const body = new PatchSponsorBodyDto(id, name);
