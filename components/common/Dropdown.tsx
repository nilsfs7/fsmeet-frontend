...
const Dropdown = ({ menus, defaultMenu, disabled = false, className, onChange }: IDropdownProps) => {
  ...
  return (
    <div className={'relative'}>
      <Menu as={'div'} className={`w-48 ${className ?? ''}`}>
        <Menu.Button
          className={`text-secondary text-md flex h-[38px] w-full cursor-pointer items-center justify-between rounded-lg border-2 border-black bg-zinc-300 px-5 py-3 font-medium hover:bg-zinc-400 ${
        ...
        {!disabled && (
          <Menu.Items className={'absolute left-0 right-0 top-0 z-10 mt-[50px] overflow-hidden rounded-lg border-[2px] border-black bg-zinc-300'}>
              ...
              {menus.map((item, index) => {
                return (
                  <Menu.Item key={index} as={Fragment}>
                    {({ active }) => (
                      <div
                        className={`${active && 'bg-zinc-300 text-white'} text-secondary text-md flex h-[38px] w-full cursor-pointer items-center px-5 font-medium  hover:bg-zinc-400`}
